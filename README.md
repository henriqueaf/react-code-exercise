![tests status](https://github.com/henriqueaf/react-code-exercise/actions/workflows/node.yml/badge.svg?branch=master)
...
You need to create a new file `.env` on project `root` directory and put your credentials into it.

```shell
$ cp /project_root_directory/.example.env /project_root_directory/.env
```

Now, just run `yarn start` to see the project running on your browser page `http://localhost:3000`
